  ...
  return (
    <div className="premium-card overflow-hidden" style={{ background: "rgba(15, 23, 42, 0.4)", backdropFilter: "blur(20px)" }}>
      <button
        ...
        className={`w-full flex items-center justify-between px-6 py-5 ${expandable ? "hover:bg-white/5 cursor-pointer" : "cursor-default"} transition-colors`}
      >
        <div className="flex items-center gap-4">
          ...
          <p className="text-[15px] font-bold text-white tracking-tight">{title}</p>
        </div>
        {expandable && (
        ...
        <div className="relative h-56 group" style={{
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100">
            <button onClick={() => coverInputRef.current?.click()}
              className="flex items-center gap-2 bg-white/20 hover:bg-white/30 backdrop-blur-md text-white text-sm font-bold px-5 py-2.5 rounded-xl transition-all border border-white/10">
              <MdCameraAlt size={18} /> {t.changeCover}
            </button>
            {user?.coverImage && (
              <button onClick={() => handleRemoveImage("cover")}
                className="flex items-center gap-2 bg-red-500/40 hover:bg-red-500/60 backdrop-blur-md text-white text-sm font-bold px-5 py-2.5 rounded-xl transition-all border border-red-500/20">
          ...
          <div className="absolute -bottom-10 left-8 group/av">
            <div className="w-24 h-24 rounded-3xl overflow-hidden shadow-2xl flex items-center justify-center text-4xl font-black text-white"
              style={{
            ...
            {editName ? (
              <div className="flex items-center gap-3 flex-1">
                <input value={name} onChange={(e) => setName(e.target.value)}
                  ...
                  className="text-2xl font-bold text-white border-b-2 outline-none bg-transparent flex-1"
                  autoFocus />
                <button onClick={handleSaveName} className="p-2 rounded-xl text-white bg-indigo-500 hover:bg-indigo-600 transition-colors">
                  <MdSave size={20} />
                </button>
                <button onClick={() => { setEditName(false); setName(user?.name); }} className="p-2 text-slate-400 hover:bg-white/5 rounded-xl">
                  <MdClose size={20} />
                </button>
              </div>
            ) : (
              <>
                ...
                <button onClick={() => setEditName(true)} className="p-2 text-slate-500 hover:bg-white/5 rounded-xl transition-all">
                  <MdEdit size={20} />
                </button>
              </>
            )}
              ...
              {Object.entries(LANGUAGES).map(([code, info]) => (
                <button key={code} onClick={() => changeLang(code)}
                  className={`w-full flex items-center justify-between px-5 py-3.5 rounded-xl border-2 transition-all font-bold text-sm ${
                    lang === code ? "border-indigo-500 bg-indigo-500/10 text-white" : "border-white/5 bg-white/3 text-slate-500 hover:border-white/10"
                  }`}>
                  <div className="flex items-center gap-3">
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${lang === code ? "bg-indigo-500 text-white" : "bg-white/10"}`}>{info.code}</span>
                    {info.label}
                  </div>
              ...
              {["currentPassword", "newPassword", "confirm"].map((key) => (
                <div key={key}>
                  <input type="password" 
                    ...
                    className="w-full bg-white/3 border border-white/8 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-indigo-500 transition-all" />
                </div>
              ))}
