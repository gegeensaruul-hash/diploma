...
import { MdAdd, MdSend, MdClose, MdExitToApp, MdDelete, MdPeople, MdArrowBack, MdChat, MdExplore, MdPersonAdd, MdCheck, MdMailOutline } from "react-icons/md";
...
export default function GroupChat({ onClose }) {
  ...
  const { data: invitesData, refetch: refetchInvites } = useGetInvitesQuery();
  ...
  const handleAddMember = async () => {
    ...
    try {
      const res = await addMember({ id: activeRoom.id, email: inviteEmail }).unwrap();
      setInviteEmail("");
      setShowAddMember(false);
      toast.success(res.message || "Урилга илгээгдлээ!");
      refetchRooms();
    } catch (e) { toast.error(e?.data?.message || "Алдаа гарлаа"); }
  };
  ...
  const handleAcceptInvite = async (inviteId) => {
    try {
      ...
      toast.success("Урилга зөвшөөрөгдлөө!");
      refetchInvites();
      refetchRooms();
  ...
  const handleDeclineInvite = async (inviteId) => {
    try {
      ...
      toast.success("Урилга татгалзлаа");
      refetchInvites();
      refetchRooms();
    } catch (e) { toast.error(e?.data?.message || "Алдаа"); }
  };
      ...
      <div className="h-16 px-5 border-b border-black/5 dark:border-white/5 flex items-center justify-between flex-shrink-0 bg-slate-900/40 backdrop-blur-md">
        <div className="flex items-center gap-3">
          {activeRoom ? (
            ...
            <button onClick={() => setShowInvites(false)} className="text-slate-400 hover:text-stone-900 dark:text-white transition-colors">
              <MdArrowBack size={20} />
            </button>
          ) : (
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {invites.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-4 text-slate-600">
                ...
                <p className="text-sm font-medium text-center">Одоогоор урилга байхгүй байна.</p>
              </div>
            ) : (
              invites.map((invite) => (
                <div key={invite.id} className="chat-room-card rounded-2xl px-4 py-4 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-[14px] font-bold text-stone-900 dark:text-white truncate mb-0.5">
                      # {invite.ChatRoom?.name || "Room"}
                    </p>
                    <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                      {invite.ChatRoom?.members?.length || 0} Members
                    </p>
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-black/5 dark:bg-white/5 hover:bg-red-500/20 text-slate-400 hover:text-red-400 text-[11px] font-black uppercase rounded-xl border border-black/10 dark:border-white/10 transition-all"
                    >
                      Decline
                    </button>
                  </div>
